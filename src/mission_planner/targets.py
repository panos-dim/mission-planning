...
class GroundTarget:
    ...
    priority: int = 5  # Target priority (1=best, 5=lowest) for scheduling
            ...
            if self.mission_type == "imaging":
                ...
                if imaging_type == "optical":
                    self.sensor_fov_half_angle_deg = (
                        1.0  # Realistic for WorldView-3/4, SkySat
                    )
                else:
    ...
    def _validate_elevation_mask(self) -> None:
        ...
        if not 0 <= self.elevation_mask <= 90:
            raise ValueError(
                f"Invalid elevation mask: {self.elevation_mask}. Must be between 0 and 90 degrees."
            )
    ...
    def _validate_mission_type(self) -> None:
        ...
        if self.mission_type not in valid_types:
            raise ValueError(
                f"Invalid mission type: {self.mission_type}. Must be one of {valid_types}."
            )
...
class TargetManager:
    ...
    def add_target(self, target: GroundTarget) -> None:
        ...
        if target.name in existing_names:
            logger.warning(
                f"Target with name '{target.name}' already exists. Adding anyway."
            )
    ...
    def get_targets_in_region(
        ...
        logger.info(
            f"Found {len(nearby_targets)} targets within {radius_km}km of "
    ...
    def save_to_file(self, file_path: str) -> None:
        ...
        try:
            ...
            with open(file_path, "w") as f:
                json.dump(
                    {"targets": targets_data, "count": len(targets_data)}, f, indent=2
                )
    ...
    def load_from_file(cls, file_path: str) -> "TargetManager":
        ...
        try:
            ...
            targets = [
                GroundTarget.from_dict(target_data)
    ...
    def __iter__(self) -> Iterator["GroundTarget"]:
