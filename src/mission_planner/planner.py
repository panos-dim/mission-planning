...
class MissionPlanner:
    ...
    def __init__(
        self, satellite: SatelliteOrbit, targets: Optional[List[GroundTarget]] = None
    ) -> None:
        """
    ...
    def compute_passes(
        ...
        self.visibility_calculator = VisibilityCalculator(
            self.satellite, use_adaptive=use_adaptive
        )

        mode = "parallel" if use_parallel else "serial"
        algo = "adaptive" if use_adaptive else "fixed-step"
        logger.info(
            f"Computing passes from {start_time} to {end_time} "
    ...
    def get_mission_summary(
        self, passes: Dict[str, List[PassDetails]]
    ) -> Dict[str, Any]:
        """
        ...
        total_contact_time = (
            sum((p.end_time - p.start_time).total_seconds() for p in all_passes) / 60
        )
        ...
        logger.info(
            f"Generated mission summary: {total_passes} passes, "
    ...
    def create_mission_visualization(
        ...
        create_mission_overview_plot(
            self.satellite, targets, start_time, duration_hours, output_file
        )
    ...
    def _create_pass_timeline(
        self, start_time: datetime, duration_hours: float, base_filename: str
    ) -> None:
    ...
    def _create_detailed_pass_visualization(
        ...
        for i, pass_detail in enumerate(all_passes):
            ...
            if imaging_targets:
                ...
                window_start_track = self.satellite.get_ground_track(
                    pass_detail.start_time, pass_detail.start_time, time_step_minutes=1
                )
                window_end_track = self.satellite.get_ground_track(
                    pass_detail.end_time, pass_detail.end_time, time_step_minutes=1
                )

                # Circle for imaging window start
                if window_start_track:
                    ax.plot(
                        window_start_track[0][2],
                        window_start_track[0][1],
                        "o",
                        color=colors[i],
                        markersize=12,
                ...
                if window_end_track:
                    ax.plot(
                        window_end_track[0][2],
                        window_end_track[0][1],
                        "s",
                        color=colors[i],
                        markersize=12,
                        markeredgecolor="black",
                ...
                pass_start_track = self.satellite.get_ground_track(
                    pass_start, pass_start, time_step_minutes=1
                )
                pass_end_track = self.satellite.get_ground_track(
                    pass_end, pass_end, time_step_minutes=1
                )

                if pass_start_track:
                    ax.plot(
                        pass_start_track[0][2],
                        pass_start_track[0][1],
                        "o",
                        color=colors[i],
                        markersize=8,
                        markeredgecolor="black",
        ...
        if imaging_targets:
            ...
            total_opportunities = sum(
                len(getattr(pass_detail, "_imaging_opportunities", []))
                for pass_detail in all_passes
            )
            pass_text = "Pass" if len(all_passes) == 1 else "Passes"
            opp_text = (
                "Max Imaging Opportunity"
                if total_opportunities == 1
                else "Max Imaging Opportunities"
            )
        ...
        if imaging_targets:
            explanation = (
                "• Each colored line = satellite ground track\n"
    ...
    def _create_imaging_legend(self, ax, all_passes, targets):
        ...
        for i, pass_detail in enumerate(all_passes):
            ...
            duration = (
                imaging_window_end - imaging_window_start
            ...
            opp_text = (
                "imaging opportunity"
                if len(pass_opps) == 1
                else "imaging opportunities"
            )
            label = (
                f"Pass {i+1}: {len(pass_opps)} {opp_text}\n"
                f"  {time_range} ({duration_str})"
            )

            legend_elements.append(
                Line2D([0], [0], color=colors[i], linewidth=3, label=label)
            )
    ...
    def _add_imaging_opportunities_to_plot(self, ax, targets):
        ...
        if all_opportunities:
            logger.info(
                f"Imaging mission: {len(all_opportunities)} potential opportunities found (not visualized)"
            )
    ...
    def run_mission_analysis(
        ...
        logger.info(
            f"Running complete mission analysis from {start_time} to {end_time}"
        )
        ...
        if output_dir:
            ...
            self._create_detailed_pass_visualization(
                start_time, duration_hours, str(viz_file)
            )
