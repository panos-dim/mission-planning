...
export const WEIGHT_PRESETS: Record<string, { 
  priority: number
  geometry: number
  timing: number
  label: string
  ...
  balanced: { priority: 40, geometry: 40, timing: 20, label: 'Balanced', desc: 'Equal priority & geometry' },
  priority_first: { priority: 70, geometry: 20, timing: 10, label: 'Priority', desc: 'High-importance targets' },
  quality_first: { priority: 20, geometry: 70, timing: 10, label: 'Quality', desc: 'Best imaging geometry' },
  urgent: { priority: 60, geometry: 10, timing: 30, label: 'Urgent', desc: 'Time-critical collection' },
  archival: { priority: 10, geometry: 80, timing: 10, label: 'Archival', desc: 'Best quality for archive' }
}

export const ALGORITHMS = [
  { id: 'first_fit', name: 'First-Fit (Chronological)', description: 'Greedy chronological selection, O(n)' },
  { id: 'roll_pitch_first_fit', name: 'First-Fit (Roll+Pitch)', description: '2D slew with roll and pitch, O(n)' },
  { id: 'best_fit', name: 'Best-Fit (Global Geometry)', description: 'Best geometry per target, O(n log n)' },
  { id: 'roll_pitch_best_fit', name: 'Best-Fit (Roll+Pitch)', description: '2D slew + global best geometry, O(n log n)' }
]

export const ALGORITHM_ORDER = ['first_fit', 'roll_pitch_first_fit', 'best_fit', 'roll_pitch_best_fit']
...
export function usePlanningState() {
  ...
  const runPlanning = useCallback(async (algorithms: string[]) => {
    ...
    try {
      ...
      if (data.success && data.results) {
        ...
        if (Object.keys(data.results).length > 1) {
          console.log('%c📊 Algorithm Comparison', 'color: #00BCD4; font-weight: bold; font-size: 14px')
          console.table(
  ...
  const navigateToScheduledTime = useCallback((scheduledTime: string) => {
  ...
  const exportToCsv = useCallback((algorithm: string) => {
    if (!results || !results[algorithm]) return
    
    const result = results[algorithm]
    const csv = [
      ['Opportunity ID', 'Satellite', 'Target', 'Time (UTC)', 'Off-Nadir (°)', 'Roll (°)', 'Pitch (°)', 'Slew (s)', 'Value'].join(','),
  ...
  const exportToJson = useCallback((algorithm: string) => {
    if (!results || !results[algorithm]) return
