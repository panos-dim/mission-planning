...
export const SlewCanvasOverlay: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const animationTime =
        ((currentTime - animationStartTime.current) / 3000) % 1; // 3 second cycle
      ...
      slewArcs.forEach((arc, index) => {
        ...
        const fromScreen = SceneTransforms.worldToWindowCoordinates(
          viewer.scene,
          fromPos,
        );
        const toScreen = SceneTransforms.worldToWindowCoordinates(
          viewer.scene,
          toPos,
        );
        ...
        // Parse color
        const colorMatch = arc.color.match(
          /rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*([\d.]+))?\)/,
        );
        ...
        if (lineLength > 20) {
          ...
          const lightX =
            fromScreen.x + (toScreen.x - fromScreen.x) * lightProgress;
          const lightY =
            fromScreen.y + (toScreen.y - fromScreen.y) * lightProgress;
      ...
      footprints.forEach((footprint) => {
        ...
        const offsetScreen = SceneTransforms.worldToWindowCoordinates(
          viewer.scene,
          offsetPos,
        );
        const radiusPixels = offsetScreen
          ? Math.abs(screenPos.x - offsetScreen.x)
          : 30;

        // Parse color
        const colorMatch = footprint.color.match(
          /rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*([\d.]+))?\)/,
        );
