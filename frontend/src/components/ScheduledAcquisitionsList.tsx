 ...
 * - Lock toggle controls (hard/soft/none)
 * - Bulk lock actions
 * - State indicators
...
export default function ScheduledAcquisitionsList({
  ...
  useEffect(() => {
    if (highlightedAcquisitionIds.length > 0) {
      ...
      if (satellitesToExpand.size > 0) {
        setExpandedSatellites(
          (prev) => new Set([...prev, ...satellitesToExpand]),
        );
      }
    }
  ...
  // Stats
  const stats = useMemo(() => {
    const byLock: Record<string, number> = { none: 0, soft: 0, hard: 0 };
  ...
  const handleLockChange = useCallback(
    async (acquisitionId: string, newLevel: LockLevel) => {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Failed to update lock level",
        );
      }
    },
  ...
  const handleBulkLock = useCallback(
    async (level: LockLevel) => {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Failed to update lock levels",
        );
      } finally {
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to hard-lock committed acquisitions",
      );
    } finally {
      setBulkLoading(false);
    }
    ...
    <div className="bg-gray-900 rounded-lg border border-gray-700 overflow-hidden flex flex-col h-full">
      ...
      <div className="flex items-center gap-4 px-4 py-2 bg-gray-800/50 border-b border-gray-700 text-xs">
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-400">{stats.byLock.none || 0} none</span>
          <span className="text-yellow-400">{stats.byLock.soft || 0} soft</span>
          <span className="text-red-400">{stats.byLock.hard || 0} hard</span>
        </div>
              ...
              <div key={satId}>
                ...
                {expandedSatellites.has(satId) && (
                  <div className="bg-gray-900/50" ref={listContainerRef}>
                    {satAcqs.map((acq) => {
                      const isHighlighted = highlightedAcquisitionIds.includes(
                        acq.id,
                      );
                        ...
                        >
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <span
                                className={`text-xs ${getStateColor(acq.state)}`}
                              >
                                {acq.state}
                              </span>
                            </div>
