...
const TLEInput: React.FC<TLEInputProps> = ({ tle, onChange }) => {
  ...
  const { data: tleSourcesData, isLoading: isLoadingSources } = useTLESources()
  const tleSources = tleSourcesData?.sources ?? []

  const [selectedSource, setSelectedSource] = useState<string>('celestrak_active')
  ...
  const loadSampleTLE = () => {
    ...
    onChange({
      name: 'ICEYE-X44',
      line1: '1 99999U 24001A   25225.50000000  .00000000  00000-0  00000-0 0  9990',
      line2: '2 99999  97.4000 180.0000 0001000  90.0000 270.0000 15.20000000000000',
    })
  }
  ...
  return (
    <div className="space-y-4">
            ...
            <div className="max-h-48 overflow-y-auto custom-scrollbar">
              ...
              <div className="space-y-1">
                {searchResults.map((satellite, index) => (
                  ...
                  >
                    <div className="text-sm text-white font-medium">{satellite.name}</div>
      ...
      <div className="space-y-3">
        <div>
          <label className="block text-xs font-medium text-gray-400 mb-1">Satellite Name</label>
          <input
            type="text"
            value={tle.name}
        ...
        <div>
          <label className="block text-xs font-medium text-gray-400 mb-1">Line 1</label>
          <input
            type="text"
            value={tle.line1}
        ...
        <div>
          <label className="block text-xs font-medium text-gray-400 mb-1">Line 2</label>
          <input
            type="text"
            value={tle.line2}
      ...
      {state.validationResult && (
        ...
        >
          <div className="flex items-center space-x-2">
            ...
            <span
              className={`text-sm ${
                state.validationResult?.valid ? 'text-green-400' : 'text-red-400'
              }`}
            >
          ...
          {state.validationResult?.valid && state.validationResult?.current_position && (
            <div className="text-xs space-y-1">
              <div>
                <span className="text-gray-400">Current Position:</span>
                <br />
                <span>
                  {state.validationResult.current_position.latitude?.toFixed(2)}
                  °, {state.validationResult.current_position.longitude?.toFixed(2)}
                  °, {state.validationResult.current_position.altitude_km?.toFixed(1)}
                  km
                </span>
              </div>
              <div>
