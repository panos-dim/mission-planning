...
const ERROR_TEMPLATES: Record<
  string,
  Omit<MappedError, "code" | "technical">
> = {
  // Analysis errors
  [ERROR_CODES.NO_SATELLITES]: {
    message: "No satellites selected",
    suggestion:
      "Go to Admin Panel to select satellites for your constellation.",
    severity: "error",
  },
  [ERROR_CODES.NO_TARGETS]: {
    message: "No targets configured",
    suggestion:
      "Add at least one target location in the Mission Analysis panel.",
    severity: "error",
  },
  ...
  [ERROR_CODES.NO_OPPORTUNITIES]: {
    ...
    suggestion:
      "Try extending the time range or adjusting satellite constraints.",
    severity: "warning",
  },
  [ERROR_CODES.ANALYSIS_FAILED]: {
    message: "Mission analysis failed",
    suggestion:
      "Check your configuration and try again. Contact support if the issue persists.",
    severity: "error",
  },
  ...
  [ERROR_CODES.NO_OPPORTUNITIES_TO_SCHEDULE]: {
    ...
    suggestion: "Run Mission Analysis first to generate imaging opportunities.",
    severity: "warning",
  },
  [ERROR_CODES.PLANNING_FAILED]: {
    message: "Planning algorithm failed",
    suggestion:
      "Check target constraints and try again with different parameters.",
    severity: "error",
  },
  ...
  [ERROR_CODES.CONFLICT_BLOCK]: {
    ...
    suggestion:
      "Use Repair mode to resolve scheduling conflicts before committing.",
    severity: "error",
  },
  ...
  [ERROR_CODES.NOT_FOUND]: {
    ...
    suggestion:
      "The requested data could not be found. It may have been deleted.",
    severity: "warning",
  },
...
function detectErrorCode(error: unknown, context?: string): string {
  ...
  if (isApiError(error)) {
    ...
    const detail =
      data?.detail?.toLowerCase() || data?.message?.toLowerCase() || "";
