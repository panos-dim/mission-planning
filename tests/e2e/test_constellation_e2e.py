...
def _json_dict_response(resp: requests.Response) -> dict[str, Any]:
    data = resp.json()
...
def test_large_constellation() -> bool:
    ...
    if result.get("data"):
        ...
        by_sat: DefaultDict[str, int] = defaultdict(int)
...
def test_long_duration() -> bool:
    ...
    if result.get("data"):
        md = result["data"]["mission_data"]
        print(
            f"Duration: {md['duration_hours']:.0f} hours ({md['duration_hours']/24:.0f} days)"
        )
        print(f"Total passes: {md['total_passes']}")
...
def test_single_satellite_backward_compat() -> bool:
    ...
    payload = {
        "tle": {"name": "HAWK-7C", **SATELLITES["HAWK-7C"]},
        ...
        for algo, result in plan_result["results"].items():
            ...
            if schedule:
                ...
                for i, opp in enumerate(schedule[:5]):
                    print(
                        f"    {i+1}. {opp.get('target', 'N/A')} @ {opp.get('imaging_time', 'N/A')[:16]}"
                    )
    else:
...
def test_czml_generation() -> bool:
    ...
    if result.get("data"):
        ...
        sat_packets = [
            p
            for p in czml
            if p.get("id", "").startswith("sat_")
            and "ground_track" not in p.get("id", "")
        ]
        ...
        other_packets = [
            p
            for p in czml
            if p not in doc_packets + sat_packets + track_packets + target_packets
        ]
