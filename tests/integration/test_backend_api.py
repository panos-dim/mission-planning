...
def test_legacy_pointing_angle():
...
def test_communication_mission():
...
def test_wide_fov_tight_bus_scenario():
    ...
    if response.status_code == 200:
        data = response.json()
        print(f"✅ Mission planned successfully")
        print(f"   - Found {len(data.get('passes', []))} passes (wide FOV should find more)")
    else:
        print(f"❌ Request failed: {response.text}")
        return False
